 ...
 */
export function TeamBadge({ team, size = "md", className = "", }) {
    const [failed, setFailed] = useState(false);
    const mark = teamMark(team);
    const hue = teamHue(team.name);
    if (team.logo && !failed) {
        return (_jsx("img", { src: team.logo, alt: "", loading: "lazy", onError: () => setFailed(true), className: `${SIZES[size]} shrink-0 rounded-full object-contain ${className}` }));
    }
    return (_jsx("span", { "aria-hidden": "true", className: `${SIZES[size]} u-display grid shrink-0 place-items-center rounded-full
                  text-ink-bright ring-1 ring-inset ring-white/10 ${className}`, style: {
 ...
 */
export function PlayerAvatar({ name, playerId, photo, size = "md", className = "", }) {
    const [failed, setFailed] = useState(false);
    const src = photo ?? (playerId === undefined ? undefined : playerPhotoUrl(playerId));
    if (src && !failed) {
        return (_jsx("img", { src: src, alt: "", loading: "lazy", onError: () => setFailed(true), className: `${SIZES[size]} shrink-0 rounded-full object-cover ${className}` }));
    }
    return (_jsx("span", { "aria-hidden": "true", className: `${SIZES[size]} u-display grid shrink-0 place-items-center rounded-full
                  bg-ember/[0.12] text-ember ring-1 ring-inset ring-ember/25 ${className}`, children: initials(name) }));
}
/**
...
export function Chip({ children, tone = "neutral", }) {
    if (tone === "live") {
        return (_jsxs("span", { className: "u-stamp u-eyebrow inline-flex items-center gap-1.5 bg-ink px-2.5 py-1 text-[0.625rem] text-ember-bright", children: [_jsx("span", { className: "h-1.5 w-1.5 animate-live rounded-full bg-ember-bright" }), children] }));
    }
    const tones = {
    ...
    return (_jsx("span", { className: `u-eyebrow inline-flex items-center gap-1.5 rounded-full px-2.5 py-1
                  text-[0.625rem] ring-1 ${tones[tone]}`, children: children }));
}
