 ...
 */
export function ScoreTicker({ matches }: { matches: Match[] }) {
    ...
    >
      ...
      >
        {track.map((match, index) => (
          <TickerItem key={`${match.id}-${index}`} match={match} duplicate={index >= matches.length} />
        ))}
      </div>
    </div>
  );
...
function TickerItem({ match, duplicate }: { match: Match; duplicate: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span className="tnum u-display text-sm text-ink-bright">
        {played ? `${home}–${away}` : kickoffTime(match.utcDate)}
      </span>
