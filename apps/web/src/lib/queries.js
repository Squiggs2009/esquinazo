...
import { ApiError, getFixtureDetail, getFixtures, getFixturesByDate, getNews, getPlayerStatistics, getSquad, getStandings, getTeams, getTransfers, isLive, } from "./api";
/**
 ...
 */
export function useFixtureDetail(fixtureId) {
    return useQuery({
        queryKey: ["fixture-detail", fixtureId],
        queryFn: () => getFixtureDetail(fixtureId),
 ...
 */
export function usePlayerStatistics(playerId, season) {
    return useQuery({
        queryKey: ["player-statistics", playerId, season],
        queryFn: () => getPlayerStatistics(playerId, season),
        enabled: typeof playerId === "number" && typeof season === "number",
        staleTime: 60 * MINUTE,
        retry: retryPolicy,
    });
}
export function useTransfers(playerId) {
    return useQuery({
        queryKey: ["transfers", playerId],
        queryFn: () => getTransfers(playerId),
